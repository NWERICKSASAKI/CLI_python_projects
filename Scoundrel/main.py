...
class Card():
    ...
    def get_naipe_str(self, naipe:str):
        match naipe:
            case 1: return ['♣︎','-']
            case 2: return ['♥︎','+']
            case 3: return ['♠︎','-']
...
class Gam():
    ...
    def _change_lp(self, n:int):
        self.lp += n
        if self.lp <= 0:
            self._gam_over(self.lp)
        elif self.lp > 20:
            self.lp = 20

    def _gam_over(self, score:int):
        self._gam = False
        if score > 0:
        ...
        if len(self.dungeon) == 0:
            if len(self.room) == 0:
                ...
                if last_card.naipe == 2:
                    self._gam_over(self.lp + last_card.value)
                    return True
    ...
    def _do_command(self, command):
        ...
        match list_commands[0]:
            ...
            case 'reset' | 'restart' | 'r':
                return self.reset_gam(list_commands)
            case 'manual' | 'm':
    ...
    def reset_gam(self, list_commands):
        if len(list_commands)>1:
